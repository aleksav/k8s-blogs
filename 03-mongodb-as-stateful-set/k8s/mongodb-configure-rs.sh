...
kubectl -n mongodb exec mongo-0 -c mongod-container -- mongo --eval 'rs.initiate({_id: "rs0", version: 1, members: [ {_id: 0, host: "mongo-0.mongo.mongodb.svc.cluster.local:27017"}, {_id: 1, host: "mongo-1.mongo.mongodb.svc.cluster.local:27017"}, {_id: 2, host: "mongo-2.mongo.mongodb.svc.cluster.local:27017"} ]});'

# Wait a bit until the replica set should have a primary ready
echo "Waiting for the Replica Set to initialise..."
sleep 30
kubectl -n mongodb exec mongo-0 -c mongod-container -- mongo --eval 'rs.status();'

# Create the admin user (this will automatically disable the localhost exception)
echo "Creating user: 'main_admin'"
kubectl -n mongodb exec mongo-0 -c mongod-container -- mongo --eval 'db.getSiblingDB("admin").createUser({user:"root",pwd:"'"${1}"'",roles:[{role:"root",db:"admin"}]});'
echo
